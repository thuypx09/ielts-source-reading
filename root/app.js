...
function resultView(id) { const data = dataCache.get(id); const record = getRecord(id); const answerItems = data.answers.passages.flatMap(item => item.answers || []); const correct = answerItems.filter(item => answersMatch(record.answers[item.question_number], item.answer)).length; const incomplete = answeredCount(record) < answerItems.length; app.innerHTML = shell(`<main class="content"><div class="detail-layout"><div class="result-actions"><button class="back-link" data-action="detail" data-id="${id}">← Exam details</button>${incomplete ? `<button class="primary-btn" data-action="continue" data-id="${id}">Continue exam</button>` : ''}</div><section class="detail-panel"><div class="eyebrow">Review / ${id}</div><h1>${correct}/${answerItems.length}<br><em>answers aligned.</em></h1><p style="color:var(--muted)">A quiet record of this attempt. Use the breakdown below to revisit the questions that need another pass.</p><div class="result-grid" style="margin-top:30px"><div class="result-card"><h3>Answer review</h3><div class="result-list">${answerItems.map(item => { const user = record.answers[item.question_number] || '—'; const isCorrect = answersMatch(user, item.answer); return `<div class="result-item ${isCorrect ? 'correct' : ''}"><b>${item.question_number}</b> ${escapeHtml(user)}<br><small>Key: ${escapeHtml(item.answer)}</small></div>`; }).join('')}</div></div></div></section></div></main>`, 'detail'); }
...
document.addEventListener('click', async event => {
  ...
  if (action === 'close-selection') { selectedRange = null; selectedQuote = ''; document.querySelector('#selection-tools')?.setAttribute('aria-hidden', 'true'); }
