...
function statusFor(id) { const record = getRecord(id); const total = answerCountForExam(id); const count = answeredCount(record); return record.completed || count >= total ? 'completed' : count ? 'in-progress' : 'not-started'; }
...
function homeView() {
  ...
  return shell(`<main class="content"><section class="hero"><div><div class="eyebrow">Quiet practice, clear progress</div><h1>Read closely.<br><em>Think clearly.</em></h1><p>A focused home for your IELTS Reading sessions. Choose a set, keep your place, and build a record of the work you have done.</p></div><div class="hero-stats"><div class="stat"><strong>${finished}</strong><span>completed</span></div><div class="stat"><strong>${active}</strong><span>in progress</span></div></div></section><div class="toolbar"><label class="search"><span>⌕</span><input data-search placeholder="Find an exam" value="${escapeHtml(query)}" /></label><div class="filters">${[['all','All sets'],['not-started','Not started'],['in-progress','In progress'],['completed','Completed']].map(([key, label]) => `<button class="filter-btn ${filter === key ? 'active' : ''}" data-filter="${key}">${label}</button>`).join('')}</div></div><section class="exam-grid">${visible.length ? visible.map(examCard).join('') : '<div class="empty">No exam sets match that filter.</div>'}</section></main>`);
}
...
function examView(id, passageIndex = 0) {
  const data = dataCache.get(id); const record = getRecord(id); const passage = passageFor(data, passageIndex); const questionData = questionsFor(data, passageIndex); const nums = questionNumbersForPassage(data, passageIndex); const seconds = record.seconds || 0; const timerPercent = Math.max(0, Math.min(100, (3600 - seconds) / 36)); const canFinish = allAnswersProvided(data, record);
  ...
  app.innerHTML = `<div class="exam-screen"><header class="exam-header"><div class="exam-controls"><div class="exam-title">IELTS Reading <span class="card-index">/ ${id}</span></div><div class="timer"><strong id="timer-value">${formatTime(seconds)}</strong><span class="progress-track"><span class="progress-fill" style="width:${timerPercent}%"></span></span></div><div class="header-actions"><button class="ghost-btn" data-action="exit" data-id="${id}">Save & exit</button>${canFinish ? `<button class="primary-btn" data-action="finish" data-id="${id}">Finish</button>` : ''}</div></div><nav class="passage-tabs">${data.passages.passages.map((item, index) => `<button class="tab ${index === passageIndex ? 'active' : ''}" data-passage="${index}">Passage ${index + 1} · ${escapeHtml(item.passage_title || '')}</button>`).join('')}</nav><nav class="exam-nav">${nums.map(number => `<button class="jump ${record.answers[number] ? 'answered' : ''} ${record.bookmarks.includes(number) ? 'bookmarked' : ''}" data-jump="${number}">${number}${record.bookmarks.includes(number) ? '<span class="nav-star" aria-hidden="true">★</span>' : ''}</button>`).join('')}</nav></header><main class="exam-body"><section class="passage-pane" id="passage-pane"><div class="passage-content"><div class="section-kicker">Reading passage ${passageIndex + 1}</div>${passage.passage_html || '<div class="empty">No passage content available.</div>'}</div></section><section class="questions-pane" id="questions-pane">${groups || '<div class="empty">No questions found for this passage.</div>'}</section></main><div class="selection-tools" id="selection-tools" aria-hidden="true"><button class="tool-btn" data-action="highlight" data-id="${id}">Highlight</button><button class="tool-btn" data-action="selection-note" data-id="${id}">Add note</button><input id="selection-note-input" class="selection-note-input" placeholder="Note about selected text" /></div></div>`;
...
function resultView(id) { const data = dataCache.get(id); const record = getRecord(id); const answerItems = data.answers.passages.flatMap(item => item.answers || []); const correct = answerItems.filter(item => answersMatch(record.answers[item.question_number], item.answer)).length; const notes = Object.values(record.notes).map(note => typeof note === 'string' ? note : `${note.quote} — ${note.note}`); app.innerHTML = shell(`<main class="content"><div class="detail-layout"><button class="back-link" data-action="detail" data-id="${id}">← Exam details</button><section class="detail-panel"><div class="eyebrow">Review / ${id}</div><h1>${correct}/${answerItems.length}<br><em>answers aligned.</em></h1><p style="color:var(--muted)">A quiet record of this attempt. Use the breakdown below to revisit the questions that need another pass.</p><div class="result-grid" style="margin-top:30px"><div class="result-card"><h3>Answer review</h3><div class="result-list">${answerItems.map(item => { const user = record.answers[item.question_number] || '—'; const isCorrect = answersMatch(user, item.answer); return `<div class="result-item ${isCorrect ? 'correct' : ''}"><b>${item.question_number}</b> ${escapeHtml(user)}<br><small>Key: ${escapeHtml(item.answer)}</small></div>`; }).join('')}</div></div><div class="result-card"><h3>Session notes</h3><p style="color:var(--muted);line-height:1.7">${notes.length ? notes.map(note => escapeHtml(note)).join('<br><br>') : 'No notes saved for this attempt.'}</p></div></div></section></div></main>`, 'detail'); }
...
function applySelection(kind, note = '') {
  ...
  try { mark.appendChild(selectedRange.extractContents()); selectedRange.insertNode(mark); } catch { toast('Select text within one paragraph.'); return; }
...
function captureSelection() { if (route.name !== 'exam') return; const selection = window.getSelection(); const text = selection?.toString().trim(); const anchor = selection?.anchorNode; if (!text || !anchor || !document.querySelector('#passage-pane')?.contains(anchor)) return; selectedRange = selection.getRangeAt(0).cloneRange(); selectedQuote = text; document.querySelector('#selection-tools')?.setAttribute('aria-hidden', 'false'); }
