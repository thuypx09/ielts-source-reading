...
document.addEventListener('click', async event => {
  ...
  if (action === 'toggle-header') { document.querySelector('.exam-screen')?.classList.toggle('header-collapsed'); const toggle = document.querySelector('[data-action="toggle-header"]'); const collapsed = document.querySelector('.exam-screen')?.classList.contains('header-collapsed'); if (toggle) { toggle.textContent = collapsed ? '▼' : '▲'; toggle.setAttribute('aria-label', collapsed ? 'Expand exam navigation' : 'Collapse exam navigation'); toggle.title = collapsed ? 'Expand exam navigation' : 'Collapse exam navigation'; } }
  if (action === 'scroll-top') { document.querySelector('.exam-body')?.scrollTo({ top: 0, behavior: 'smooth' }); }
